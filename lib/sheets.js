...
class SheetsService {
  constructor() {
    ...
    this.cache = {};
    this.cacheTTL = {
      default:      60  * 1000, // 60 segundos (leituras gerais)
      quadro:       5  * 60 * 1000, // 5 minutos (muda pouco)
      qlp:          5  * 60 * 1000, // 5 minutos
      mapaCarga:    60 * 1000,      // 60 segundos
      buffer:       10 * 1000,      // 10 segundos (muda com ações do usuário)
      base:         30 * 1000,      // 30 segundos
      usuarios:     10 * 60 * 1000, // 10 minutos (raramente muda)
    };
  }
    ...
    try {
      ...
      for (const linha of dados) {
        ...
        for (const row of rowsExistentes) {
          if (
            String(row.get('Supervisor') || '').trim() === sup &&
            String(row.get('Data')       || '').trim() === hoje &&
            String(row.get('Matricula')  || '').trim() === matricula
          ) {
      ...
      // Invalida cache da base após salvar
